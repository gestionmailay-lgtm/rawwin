...
async function run() {
  try {
    ...
    const mdsNamespaces = ["mds", "mds-administrator", "metadatastore", "metadata-store"];
    const subpaths = [
      "measuringpoints",
      "measuring-points",
      "variables",
    ...
    console.log("Fuzzing MDS namespaces...");
    for (const ns of mdsNamespaces) {
      for (const sub of subpaths) {
        const paths = [
          `/api/${ns}/${sub}`,
          `/api/${ns}/sites/${siteId}/${sub}`,
          `/api/sites/${siteId}/${ns}/${sub}`
        ];
